...
class JointVelocityController : public controller_interface::Controller<hardware_interface::PositionJointInterface> {
  bool init(hardware_interface::PositionJointInterface *hw, ros::NodeHandle &n) {
    ...
    for (size_t i = 0; i < joint_handles_.size(); i++) {
        prev_position_commanded_[i] = joint_handles_.at(i).getPosition();
    }
  ...
  void update(const ros::Time &time, const ros::Duration &period) {
    ...
    for (size_t i = 0; i < joint_handles_.size(); i++) {
      double position_command = prev_position_commanded_[i] + command_.at(i)*(t_cur - t_prev);
...
private:
  ...
  std::array<double,7> prev_position_commanded_;
};
